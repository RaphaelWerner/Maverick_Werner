...
namespace Maverick.WebApi.Controllers
{
    ...
    public class FilmesController : ApiController
    {
        ...
        [Route("Search/People")]
        [HttpGet, AllowAnonymous]
        public async Task<ActionResult> GetPessoas([FromQuery] string query)
        {
            string api_key = "2554f7f68bc8fb9bda393f7eac9c54cf";
            Object pessoas = new Object();
            using (var client = new HttpClient())
            {
                HttpResponseMessage response = await client.GetAsync("https://api.themoviedb.org/3/search/person?api_key=" + api_key + "&query=" + query);
                if (response.IsSuccessStatusCode)
                {
                    pessoas = response.Content.ReadAsAsync<Object>().Result;
                }
            }
            return Ok(JsonConvert.SerializeObject(pessoas));
        }
